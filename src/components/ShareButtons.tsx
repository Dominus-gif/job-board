...
/**
 * Share a job to major networks with a promotional message that also markets
 * AnywhereJobs. LinkedIn/Facebook pull the page's Open Graph card (see the
 * job route's `opengraph-image`), so the shared link renders as a rich post.
 */
    ...
    <div className="flex flex-wrap items-center gap-2">
      {networks.map((n) => (
        <a
          ...
          aria-label={n.label}
          title={n.label}
          className="flex h-9 w-9 items-center justify-center rounded-lg border border-ink-100 bg-white text-ink-500 transition hover:border-brand-300 hover:text-brand-700"
        >
          {n.icon}
        </a>
      ))}
      <button
        ...
        title="Copy link"
        className="flex h-9 items-center gap-1.5 rounded-lg border border-ink-100 bg-white px-3 text-sm font-medium text-ink-600 transition hover:border-brand-300 hover:text-brand-700"
      >
